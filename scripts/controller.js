...
app.controller('myCtrl', function($scope, $http) {
    ...
    $scope.supported_types = [{
        ...
        id: CONSTANTS.PERSON,
        title: CONSTANTS.PERSON
    }, {
        id: CONSTANTS.IMAGE,
        title: CONSTANTS.IMAGE
    }, {
        id: CONSTANTS.KNOWLEDGE,
        title: CONSTANTS.KNOWLEDGE
    }, {
        id: CONSTANTS.ALGORITHMS,
        title: CONSTANTS.ALGORITHMS
    }, {
        id: CONSTANTS.BOOKS,
        title: CONSTANTS.BOOKS
    }];
    ...
    function saveBookmark() {
        ...
        $http.post(API_DOMAIN + '/bookmark', $scope.bookmark).then(function successCallback(response) {
            fetchFreshData();
        }, function errorCallback(response) {});
    ...
    $scope.remove = function(id) {
        $http({
            method: 'delete',
            url: 'http://localhost:5000/remove_bookmark/' + id
        }).then(function successCallback(response) {
            fetchFreshData();
        }, function errorCallback(response) {
            // called asynchronously if an error occurs
